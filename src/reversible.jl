export i_leapfrog, i_leapfrog_clean
...
@i @inline function acceleration_clean(y!::V3{T}, ra::V3{T}, rb::V3{T}, mb::Real, G) where T
    @routine @invcheckoff begin
        @zeros T d anc1 anc2 anc3 anc4
        rc ← zero(V3{T})
        d += sqdistance(ra, rb)
...
@i function i_leapfrog_clean(r::AbstractMatrix{V3{T}}, v::AbstractMatrix{V3{T}}, planets::AbstractVector{Body{T}}; G=G_year_AU, n, dt) where T
    @routine @invcheckoff begin
        nplanets ← length(planets)
        twodt ← zero(dt)
        twodt += 2 * dt
    ...
    @inbounds @invcheckoff for i=1:n
        ...
        @routine begin
            ...
            for j=1:nplanets
                for k=1:nplanets
                    if j != k
                        acceleration_clean(a[j], r[j,mod1(i,2)], r[k,mod1(i,2)], m[k], G)
                    end
                end
            end
        end
